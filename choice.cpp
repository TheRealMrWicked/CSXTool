...
bool tryParseChoice(Choice *choice, const char *contentBegin, size_t contentLength, size_t offset, uint32_t sectionPosition) {
    ...
    if (isShortUnicodeString(contentBegin + curr, contentEnd)) {
        choice->choice1 = parseUnicodeString(contentBegin + curr, contentEnd, curr + sectionPosition);
    ...
    if (isShortUnicodeString(contentBegin + curr, contentEnd)) {
        choice->choice2 = parseUnicodeString(contentBegin + curr, contentEnd, curr + sectionPosition);
