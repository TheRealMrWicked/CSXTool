...
static const size_t shortLength = 64;

UnicodeString::UnicodeString(const char *s, uint32_t l, const char *customSpace):
    length(l) {
    assert(s != 0 || l > 0);
    ...
    if (customSpace) {
        for (size_t i = 0; i < length; ++i) {
            if (usStr[2 * i] == ' ' && usStr[2 * i + 1] == 0) {
                usStr[2 * i] = customSpace[0];
                usStr[2 * i + 1] = customSpace[1];
            }
        }
    }
...
UnicodeString parseUnicodeString(const char *source, const char *sourceEnd, uint32_t position) {
    if (source + 4 >= sourceEnd) {
        std::stringstream msg;
        msg << "Unicode string out of bounds at byte 0x " << std::hex << position << ".";
